WITH audience_for_each_metric AS (
    ...
    INNER JOIN mydb.myschema.metric_definitions AS metric_definitions
    ...
    (
        SELECT
            audience_for_each_metric.iteration_id
             ...
             , CASE WHEN audience_for_each_metric.is_numeric THEN metric_events.event_value ELSE CASE WHEN metric_events.received_time IS NOT NULL THEN 1.0 ELSE NULL END END AS event_value
        FROM audience_for_each_metric

                 LEFT JOIN mydb.myschema.metric_events AS metric_events
