INSERT INTO mydb.myschema.arm_aggregates(iteration_id, metric_version_id, arm_id, dimension_name, dimension_value, count_of_units, count_of_units_with_measurements, sum_of_unit_aggregates, sum_of_squares, metric_event_last_seen, analyzed_at) (
  WITH audience_for_each_metric AS (
   SELECT
      DISTINCT
          ...
          , audience.arm_id
      FROM mydb.myschema.audience AS audience
          WHERE audience.experiment_iteration_id = 'exp-iteration-12345'
  ), unit_aggregates as (
    SELECT 
        ...
        , results.flag_key
        , results.arm_id
        , SUM(results.event_value) AS metric_event_aggregate_value
      ...
      (
          ...
          FROM audience_for_each_metric
              LEFT JOIN (SELECT * FROM mydb.myschema.events WHERE event_type = 'metric') AS metric_events
      ...
      SELECT
          'exp-iteration-12345' as iteration_id,
          'metric-version-67890' as metric_version_id,
          ua.arm_id,
          id.dimension_name,
      ...
      JOIN mydb.myschema.iteration_dimensions id
      WHERE GET_PATH(PARSE_JSON(ua.context_attributes), id.dimension_name) IS NOT NULL AND id.iteration_id = 'exp-iteration-12345'
  ) SELECT
          'exp-iteration-12345' as iteration_id,
          'metric-version-67890' as metric_version_id,
          arm_id,
          ...
          SUM(SQUARE(metric_event_aggregate_value)) AS sum_of_squares,
